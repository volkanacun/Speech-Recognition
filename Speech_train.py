...
def main(): 
    ...
    # load the train/validation data splits
    X_train, X_validation, X_test, y_train, y_validation, y_test = get_data_splits(DATA_PATH)

    # build the CNN model
    input_shape =(X_train.shape[1], X_train.shape[2], X_train.shape[3]) # we need a 3d array since we are dealing with a CNN model that deals with 3D input (#segments(sample rate), #coefficients(MFCCs) 13, 1(depth, channel, color))
